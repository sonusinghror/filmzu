...
class Conversation < ActiveRecord::Base
  ...
  scope :trash, lambda {|participant|
    participant(participant).merge(Receipt.trash.where(:deleted => false))
  }
  ...
  def direct_hire_conversation
    DirectHireProposal.where(conversation_id: self.id).first
  end

  def project_proposal_from_conversation
    ProjectProposal.where(conversation_id: self.id).first
  end
  ...
  def mark_as_deleted(participant)
    return if participant.nil?
    ActiveRecord::Base.connection.execute("UPDATE receipts SET deleted = 't' WHERE id = #{self.receipts_for(participant).first.id}")
  end
