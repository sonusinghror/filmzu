class Clients::OmniauthCallbacksController < Devise::OmniauthCallbacksController
   ...
   def linked_in
    ...
    def oauthorize(kind)
      @client = find_for_ouath(kind, env["omniauth.auth"], current_user)
      if @client
  ...
  return user
    ...
    def find_for_oauth_by_email(email, resource=nil)
       if user = Client.find_by_email(email)
      user
    else
    user = Client.new(:email => email, :password => Devise.friendly_token[0,20])
    user.save
     end
   return user
end
    ...
    def find_for_oauth_by_name(name, resource=nil)
      if user = Client.find_by_name(name)
    user
  else
   user = Client.new(:name => name, :password => Devise.friendly_token[0,20], :email => "#{UUIDTools::UUID.random_create}@host")
   user.save false
   end
  return user
 end
